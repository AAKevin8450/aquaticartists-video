"""
Process batch results for batch job 813pkj5xu97q (300 videos)
"""
import json
import sys
...
for i, nova_job_id in enumerate(nova_job_ids, 1):
    ...
    if job['batch_status'] == 'COMPLETED' and job['summary_result']:
        print(f"[{i}/{len(nova_job_ids)}] Job {nova_job_id} already completed - SKIP")
        success_count += 1
        total_cost += job.get('cost_usd', 0) or 0
        total_tokens_input += job.get('tokens_input', 0) or 0
        total_tokens_output += job.get('tokens_output', 0) or 0
        continue
...
print("=" * 80)
print(f"Success: {success_count}/{len(nova_job_ids)} ({success_count/len(nova_job_ids)*100:.1f}%)")
print(f"Failed: {fail_count}/{len(nova_job_ids)} ({fail_count/len(nova_job_ids)*100:.1f}%)")
print()
